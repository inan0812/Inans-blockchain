...
def get_inan_ca_crt_key() -> Tuple[Any, Any]:
    crt = pkg_resources.resource_string(__name__, "inan_ca.crt")
    key = pkg_resources.resource_string(__name__, "inan_ca.key")
    return crt, key
...
def generate_ca_signed_cert(ca_crt: bytes, ca_key: bytes, cert_out: Path, key_out: Path):
    ...
    new_subject = x509.Name(
        [
            x509.NameAttribute(NameOID.COMMON_NAME, "Inan"),
    ...
    cert = (
        ...
        .add_extension(
            x509.SubjectAlternativeName([x509.DNSName("scampool.org")]),
            critical=False,
        )
...
def main():
    return make_ca_cert(Path("./inan_ca.crt"), Path("./inan_ca.key"))
