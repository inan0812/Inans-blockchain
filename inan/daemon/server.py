...
from inan.cmds.init_funcs import inan_init
from inan.daemon.windows_signal import kill
from inan.server.server import ssl_context_for_root, ssl_context_for_server
from inan.ssl.create_ssl import get_mozilla_ca_crt
from inan.util.inan_logging import initialize_logging
from inan.util.config import load_config
from inan.util.json_util import dict_to_json_str
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Inan.")
    quit()
...
service_plotter = "inan plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "inan": "inan",
        "inan_wallet": "start_wallet",
        "inan_full_node": "start_full_node",
        "inan_harvester": "start_harvester",
        "inan_farmer": "start_farmer",
        "inan_introducer": "start_introducer",
        "inan_timelord": "start_timelord",
        "inan_timelord_launcher": "timelord_launcher",
        "inan_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("inan daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered INAN_ROOT
    os.environ["INAN_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with INAN_ROOT: {os.environ['INAN_ROOT']}")
...
def main() -> int:
    from inan.util.default_root import DEFAULT_ROOT_PATH
