...
ROOT = pathlib.Path(importlib.import_module("inan").__file__).absolute().parent.parent


def solve_name_collision_problem(analysis):
    """
    There is a collision between the `inan` file name (which is the executable)
    and the `inan` directory, which contains non-code resources like `english.txt`.
    We move all the resources in the zipped area so there is no
    need to create the `inan` directory, since the names collide.
...
# TODO: collapse all these entry points into one `inan_exec` entrypoint that accepts the server as a parameter

entry_points = ["inan.cmds.inan"] + [f"inan.server.start_{s}" for s in SERVERS]
...
if THIS_IS_WINDOWS:
    entry_points.extend(["aiohttp", "inan.util.bip39"])

if THIS_IS_WINDOWS:
    inan_mod = importlib.import_module("inan")
    dll_paths = ROOT / "*.dll"
...
datas.append((f"{ROOT}/inan/util/english.txt", "inan/util"))
datas.append((f"{ROOT}/inan/util/initial-config.yaml", "inan/util"))
datas.append((f"{ROOT}/inan/wallet/puzzles/*.hex", "inan/wallet/puzzles"))
datas.append((f"{ROOT}/inan/ssl/*", "inan/ssl"))
...
add_binary("inan", f"{ROOT}/inan/cmds/inan.py", COLLECT_ARGS)
add_binary("daemon", f"{ROOT}/inan/daemon/server.py", COLLECT_ARGS)

for server in SERVERS:
    add_binary(f"start_{server}", f"{ROOT}/inan/server/start_{server}.py", COLLECT_ARGS)
