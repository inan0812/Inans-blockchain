...
from inan.cmds.units import units
from inan.consensus.block_record import BlockRecord
from inan.rpc.farmer_rpc_client import FarmerRpcClient
from inan.rpc.full_node_rpc_client import FullNodeRpcClient
from inan.rpc.wallet_rpc_client import WalletRpcClient
...
from inan.util.misc import format_bytes
...
async def summary(rpc_port: int, wallet_rpc_port: int, harvester_rpc_port: int, farmer_rpc_port: int) -> None:
    ...
    if amounts is not None:
        print(f"Total inan farmed: {amounts['farmed_amount'] / units['inan']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['inan']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['inan']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'inan start wallet' and 'inan wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'inan wallet show'")
    else:
        print("Note: log into your key using 'inan wallet show' to see rewards for each key")
