...
if [ "$(id -u)" = 0 ]; then
  echo "The Inan Blockchain GUI can not be installed or run by the root user."
	exit 1
fi

# Allows overriding the branch or commit to build in inan-blockchain-gui
SUBMODULE_BRANCH=$1
...
echo ""
echo "Type 'cd inan-blockchain-gui' and then 'npm run electron &' to start the GUI."
