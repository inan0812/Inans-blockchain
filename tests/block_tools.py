...
from inan.cmds.init_funcs import create_all_ssl, create_default_inan_config
from inan.full_node.bundle_tools import (
    best_solution_generator_from_template,
    detect_potential_template_generator,
    simple_solution_generator,
)
from inan.util.errors import Err
from inan.full_node.generator import setup_generator_args
from inan.full_node.mempool_check_conditions import GENERATOR_MOD
from inan.plotting.create_plots import create_plots
...
from inan.consensus.coinbase import create_puzzlehash_for_pk, create_farmer_coin, create_pool_coin
from inan.consensus.constants import ConsensusConstants
from inan.consensus.default_constants import DEFAULT_CONSTANTS
from inan.consensus.deficit import calculate_deficit
...
from inan.consensus.pot_iterations import (
    calculate_ip_iters,
...
from inan.consensus.vdf_info_computation import get_signage_point_vdf_info
from inan.full_node.signage_point import SignagePoint
from inan.plotting.plot_tools import PlotInfo, load_plots, parse_plot_info
...
from inan.types.blockchain_format.foliage import Foliage, FoliageBlockData, FoliageTransactionBlock, TransactionsInfo
...
from inan.types.blockchain_format.reward_chain_block import RewardChainBlockUnfinished
from inan.types.blockchain_format.sized_bytes import bytes32
from inan.types.blockchain_format.slots import (
...
from inan.types.generator_types import BlockGenerator, CompressorArg
...
from inan.util.block_cache import BlockCache
from inan.util.condition_tools import ConditionOpcode, conditions_by_opcode
from inan.util.config import load_config, save_config
from inan.util.hash import std_hash
from inan.util.ints import uint8, uint16, uint32, uint64, uint128
from inan.util.keychain import Keychain, bytes_to_mnemonic
from inan.util.merkle_set import MerkleSet
from inan.util.prev_transaction_block import get_prev_transaction_block
from inan.util.path import mkdir
from inan.util.vdf_prover import get_vdf_info_and_proof
...
class BlockTools:
    ...
    def __init__(
        ...
        if len(self.pool_pubkeys) == 0 or len(self.farmer_pubkeys) == 0:
            raise RuntimeError("Keys not generated. Run `inan generate keys`")
    ...
    def init_plots(self, root_path: Path):
        ...
        try:
            ...
            args.pool_contract_address = encode_puzzle_hash(self.pool_ph, "xgen")
...
def get_plot_dir() -> Path:
    cache_path = Path(os.path.expanduser(os.getenv("INAN_ROOT", "~/.inan/"))) / "test-plots"
    mkdir(cache_path)
    return cache_path
