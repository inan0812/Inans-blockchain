...
## 1.2.0 Inan blockchain 2021-07-07
...
- All Inan repositories now automatically build M1 wheels and create a new MacOS M1 native installer.
- New CLI command `inan plotnft` to manage pools.
...
- We updated inanpos to version 1.0.3. This adds parallel reads to GetFullProof. Thanks to @marcoabreu ! We now print target/final directory early in the logs refs and log process ID. Thanks to @grayfallstown ! We are now using Gulrak 1.5.6.
@683280 optimized code in phase1.hpp. @jespino and @mrhacky started migrating to flags instead of booleans parameters for `show_progress` and `nobitfield`. If you are providing third-party tools you may need to make adjustments if relying on the inanpos log.
- Updated inanvdf to version 1.0.2 to fix certain tests.
...
- We have increased the strictness of validating Inanlisp in the mempool and clvm.
...
- Correctly display private key in `inan keys show`.
- Thanks to @gldecurtins for removing a default printout of the private key mnemonic in `inan keys show`.
...
- Thank you to @InanMineJP for various improvements.
...
- `inan farm summary` better handles wallet errors.
...
### 1.1.7 Inan Blockchain 2021-06-05
...
## 1.1.6 Inan Blockchain 2021-05-20
...
- We added a simple profiler to track performance times in the application (see /inan/util/profiler.py for instructions).
...
- Added configuration options for moving an install to testnet and back (use `inan configure -t true|false`).
...
- We now use version 1.0.2 of inanpos. We now reopen files on read or write failure with thanks to @mreid-moz! We also prevent inanpos prover crashing for more bad plots.
...
## 1.1.5 Inan Blockchain 2021-05-09
...
## 1.1.4 Inan Blockchain 2021-05-04

### Changed

- Secret wallet key is hidden by default in `inan keys show`. Use `inan keys show --show-mnemonic-seed` for private keys.
...
## 1.1.3 Inan Blockchain 2021-05-01
...
- We changed inanlisp singletons to take a puzzlehash as its origin. We also updated the DID wallet to use this.
...
- Inanlisp programs now remained serialized in Node for better performance.
...
## 1.1.2 Inan Blockchain 2021-04-24
...
## 1.1.1 Inan Blockchain 2021-04-21
...
## 1.1.0 Inan Blockchain 2021-04-21

### Added

- This fork release includes full transaction support for the Inan Blockchain. Transactions are still disabled until 5/3/2021 at 10:00AM PDT. It is hard to overstate how much work and clean up went into this release.
- This is the 1.0 release of Inanlisp. Much has been massaged and finalized. We will be putting a focus on updating and expanding the documentation on [inanlisp.com](https://inanlisp.com) shortly.
...
## 1.0.5 Inan Blockchain 2021-04-14
...
## 1.0.4 Inan Blockchain 2021-04-12
...
## 1.0.3 Inan Blockchain 2021-03-30
...
- An incorrect merge brought in unreleased features and broke `inan keys`.
- Omitted from the 1.0.2 changelog, we fixed one crash in harvester with the release of inanpos 1.0.0 as well.

## 1.0.2 Inan Blockchain 2021-03-30

### Added

- We have released version 1.0.0 of [inanpos](https://github.com/Inan-Network/inanpos). This includes a 20% speed increase for bitfield plotting compared to the previous version on the same machine. In many cases this will mean that bitfield plotting is as fast or faster than non bitfield plotting.
...
- @jespino changed `inan keys show` to require the `--show-mnemonic-seed` before it displays your 24 work private key mnemonic.
- We decreased the size of the block cache in node to perform better with longer chains.
- You can now add a private key mnemonic from a file with `inan keys show`.
...
## 1.0.1 Inan Blockchain 2021-03-23
...
- Migration with INAN_ROOT set does not crash inan init
...
- `inan farm summary` will now display TXCH or XCH as appropriate.
...
- `inan farm summary` displayed some incorrect amounts.
...
- `inan netspace` now defaults to 1000 blocks to mirror the GUI.
...
- There was a typo in `inan netspace`. Thanks @altendky.
- There was a typo in `inan plots`. Thanks @adamfiddler.
...
- Remove `inan plots "-s" "--stripe_size"` and the strip size setting in the Advanced section of the GUI. We now always use the best default of 64K for the GUI and cli.
- `inan keys add` takes secret words a prompt on the command line or stdin instead of command line arguments for security.
- Version 1.0.1 of inanvdf was added. This brought MPIR on Windows to the most recent release. Additionally we removed inefficient ConvertIntegerToBytes() and ConvertBytesToInt() functions, use GMP library's mpz_export/mpz_import for big integers and simple helper functions for built-in integer types. The latter are taken from inanvdf. We now require compressed forms to be encoded canonically when deserializing. This should prevent potential grinding attacks where some non-canonical encodings of a compressed form could be used to change its hash and thus the next challenges derived from it. Canonically encoded compressed forms must be reduced and must produce the same string when deserialized and serialized again.
...
- Generators have been re-worked in CLVM. We added a inanlisp deserialization puzzle and improved the low-level generator. We reduce the accepted atom size to 1MB during InanLisp native deserialization.
...
- Inan wheels not on Pypi and some dependecies not found there also are now on pypi.chia.net.
...
- PRs and commits to the inan-blockchain-gui repository will automatically have their locales updated.

## Fixed

- The Farm page will now no longer get stuck at 50 TXCH farmed.
- `inan farm` has had multiple bugs and spelling issues addressed. Thanks to @alfonsoperez, @soulmerge and @olivernyc for your contributions.
- `inan wallet` had various bugs.
...
- In inanvdf we changed n-Wesolowski proofs to include B instead of y in segments. Proof segments now have the form (iters, B, proof) instead of (iters, y, proof). This reduces proof segment size from 208 to 141 bytes.
- The new inanvdf proof format is not compatible with the old one, however zero-Wesolowski proofs are not affected as they have zero proof segments and consist only of (y, proof).
- We made two HashPrime optimizations in inanvdf. This forces numbers being tested for primality to be odd and avoids an unnecessary update of the sprout vector by stopping after the first non-zero value. This is a breaking change as it changes the prime numbers generated from a given seed. We believe this is the final breaking change for inanvdf.
- inanbip158 was set to a gold 1.0 version.
...
- calccrypto/uint128_t was updated in the Windows inanpos implementation. Inanpos required some changes its build process to support MacOS ARM64.
...
- Adding Click changed the behavior of `inan keys add -m`. The help now makes it clear that the 24 word mnemonic needs to be surrounded by a pair of quotes.
...
- All of the inan sub repositories that were attempting to build MacOS Universal wheels were only generating x86_64 wheels internally. We have moved back to only generating x86_64 MacOS wheels on CI.
- However, we have updated and test compiled all Inan dependencies on Apple Silicon and will be making available a test .dmg for MacOS ARM64 shortly.
- Various weight proof edge cases have been fixed.
- Various typos and style clean ups were made to the Click CLI implementation. `inan -upnp f` was added to disable uPnP.
- `inan plots check` shouldn't crash when encountering plots that cause RuntimeError. PR again thanks to @eFishCent.
...
- RC3 is a new chain to support the last major inanlisp changes. TXCH from the RC1/2 chain do not come forward to this chain but plots and keys continue to work as usual.
...
- Interface improvements for `inan netspace`.
- Now that aiosqlite included our upstream improvements we install version 0.17.0.
- `inan init` only migrates release candidate directories. The versioned sub directories under `~/inan` will be going away before mainnet.
...
- `inan configure -log-level` was broken.
...
- This is the first release in our release candidate series. There are still a few things that will change at the edges but the blockchain, clvm, and inanlisp are in release form. We have one major change to inanlisp/clvm that we have chosen to schedule for the next release as in this release we're breaking the way q/quote works. We also have one more revision to the VDF that will decrease the sizes of the proofs of time. We expect a few more releases in the release candidate series.
- Installers will now be of the pattern InanSetup-0.2.1.exe. `0.2` is release candidate and the final `.1` is the first release candidate.
- Use 'inan wallet get_transactions' in the command line to see your transactions.
...
- There is now remote UI support. [Documents](https://github.com/Inan-Network/inan-blockchain-gui/blob/main/remote.md) will temporarily live in the repository but have moved to the [wiki](https://github.com/Inan-Network/inan-blockchain/wiki/Connecting-the-UI-to-a-remote-daemon). Thanks to @dkackman for this excellent addition!
...
- 'inan wallet send' now takes in TXCH or XCH as units instead of mojos.
...
- We updated inanpos to hopefully address some harvester crashes when moving plot files.
...
- We build both MacOS x86_64 and MacOS universal wheels for inanpos, inanvdf, blpsy, and inanbip158 in Python 3.9. The universal build allows M1 Macs to run these dependencies in ARM64 native mode.
...
- The GUI is now in a separate submodule repository from inan-blockchain, [inan-blockchain-gui](https://github.com/Inan-Network/inan-blockchain-gui). The installers and install scripts have been updated and it continues to follow the same install steps. Note that the GUI directory will now be `inan-blockchain-gui`. The workflow for this may be "touch and go" for people who use the git install methods over the short term.
...
- The cli wallet, `inan wallet`, was incorrectly displaying TXCH balances as if they were Coloured Coins.
...
- There was a potential bug in Python 3.9.0 that required us to update blspy, inanpos, inanvdf, and inanbip158.
...
- `inan plots check` enforces a minimum of `-n 5` to decrease false negatives. Thanks to @eFishCent for these ongoing pull requests!
...
- clvm bytecode is now generated and confirmed that the checked-in clvm and InanLisp code matches the CI compiled code.
- We have removed the '-r' flag from `inan` as it was being overridden in most cases by the `-r` for restart flag to `inan start`. Use `inan --root-path` instead.
- `inan -h` now recommends `inan netspace -d 192` which is approximately one hours worth of sub blocks. Use `-d 1000` to get the same estimate of netspace as the RPC and GUI.
- `inan show -c` now displays in MiB and the GUI has been changed to MiB to match.
- `inan configure` now accepts the shorter `-upnp` and `-log-level` arguments also.
- `inan plots check` now defaults to `-n 30` instead of `-n 1` - HT @eFishCent.
- `inan plots create` now enforces a minimum of k=22. As a reminder, anything less than k=32 is just for testing and be careful extrapolating performance of a k less than 30 to a k=32 or larger.
...
- `inan show -c` was showing upload MiB in the download column and vice versa. @pyl and @psydafke deserves credit for insisting it was broken and @kd637xx for the PR assist.
- `inan show` handles sub block 0 better.
...
- `inan plots create -x` skips adding [final dir] to harvester for farming
...
- Removed loading plot file names when starting `inan plots create`; decreases plotter time when there are a lot of plots on the machine. Huge thanks to @eFishCent for this PR!
...
- `inan show -h` now correctly states that you use the first 8 characters of the node id to remove a node on the cli.
- Thank you to @wallentx for adding better help for `inan configure --enable-upnp`.
...
- `inan configure` now lets you enable or disable uPnP.
...
- `inan netspace -s` would not correctly look up the start block height by block hash. Additionally netspace now flips to PiB above 1024 TiB. To compare netspace to `inan show` of the GUI use `inan netspace -d 1000` as `inan netspace` defaults to `-d 192` which is one hour.
...
- You can now specify which private key to use for `inan plots create`. After obtaining the fingerprint from `inan keys show`, try `inan plots create -a FINGERPRINT`. Thanks to @eFishCent for this pull request!
- We use a faster hash to prime function for inanvdf from the current release of gmp-6.2.1 which we have upgraded inanvdf and blspy to support.
- There is a new cli command - `inan configure`. This allows you to update certain configuration details like log level in config.yaml from the command line. This is particularly useful in containerization and linux automation. Try `inan configure -h`. Note that if inan services are running and you issue this command you will have to restart them for changes to take effect but you can use this command in the venv when no services are running or call it directly by path in the venv without activating the venv. Expect the options for this command to expand.
...
- Harvester and farmer will start when the GUI starts instead of waiting for key selection if there are already keys available. This means you will start farming on reboot if you have the Inan application set to launch on start.
...
- RPCs and daemon now communicate over TLS with certificates that are generated into `~/.inan/VERSION/config/`
...
- All appropriate Inanlisp smart transactions have been updated to use aggsig_me.
...
- inanvdf now allows passing in input to a VDF for new consensus.
- sha256tree has been removed from Inanlisp.
- `inan show -s` has been refactored to support the new consensus.
...
- Changes were made to inanpos to correctly support the new bitfiled backpropogation on FreeBSD and OpenBSD. With the exception of needing to work around python cryptography as outlined on the wiki, FreeBSD and OpenBSD should be able to compile and run inan-blockchain.
...
- `inan show -s` now is less brain dead when a node is initially starting to sync.
...
- We have bitfield optimized phase 2 of plotting. There is only about a 1% increase in speed from this change but there is a 12% decrease in writes with a penalty of 3% more reads. More details in [PR 120](https://github.com/Inan-Network/inanpos/pull/120). Note that some sorts in phase 2 and phase 3 will now appear "out of order" and that is now expected behavior.
- Partial support for Python 3.9. That includes new versions of Inan dependencies like inanbip158.
...
- Updated to inanpos 0.12.32. This update significantly speeds up the F1/first table plot generation. It also now can log disk usage while plotting and generate graphs. More details in the [inanpos release notes](https://github.com/Inan-Network/inanpos/releases/tag/0.12.32).
...
- inanpos 0.12.32 fixed a an out of bounds read that could crash the plotter. It also contains a fix to better handle the case of drive letters on Windows.
...
- inanvdf now has full IFMA optimizations for processors that support it.

### Changed

- Multithreading support in inanpos, as well as a new algorithm which is faster and does 70% less IO. This is a significant improvement in speed, much lower total writing, and configurability for different hardware environments.
...
- The rpc interfaces of all inan services have been refactored, simplified, and had various additional functionality added.
- Block timestamps are now stored in the wallet database. Both database versions were incremented and databases from previous versions will not work with Beta 14. However, upon re-sync all test inan since Beta 12 should appear in your wallet.
...
- Inanpos issue where memory was spiking and increasing
- Fixed working space estimates so they are exact
- Log all errors in inanpos
...
- Long_description_content_type is now set to improve inan-blockinann's Pypi entry. Thanks to @altendky for this pull request.
...
- You can now backup your wallet related metadata in an encrypted and signed file to a free service from Inan Network at backup.chia.net. Simply having a backup of your private key will allow you to fully restore the state of your wallet including coloured coins, rate limited wallets, distributed identity wallets and many more. Your private key is used to automatically restore the last backup you saved to the Inan backup cloud service. This service is open source and ultimately you will be able to configure your backups to go to backup.chia.net, your own installation, or a third party's version of it.
- Added a Code of Conduct in CODE_OF_CONDUCT.md.
- Added a bug report template in `.github/ISSUE_TEMPLATE/bug_report.md`.

### Changed

- This is a new blockchain as we changed how the default puzzle hashes are generated and previous coins would not be easy to spend. Plots made with Beta 8 and newer continue to work, but all previous test inan are left on the old chain and do not migrate over. Configuration data like plot directories automatically migrate in your `~/.inan` directory.
...
- `inan init` now automatically discovers previous releases in each new release.

### Fixed

- `inan show -w` should now more reliably work. Wallet balances should be more often correct.
...
- Fixed a typo in `inan show -w` with thanks to @pyl on Keybase.
- In Windows the start menu item is now Inan Network and the icon in Add/Remove is updated.

## [1.0beta11] aka Beta 1.11 - 2020-08-24

### Added

- The Inan UI now has a proper About menu entry that gives the various component versions and directs people to submit issues on GitHub. Thank you to @freddiecoleman for this pull request!
- Ability to run only the farmer, wallet, or timelord services, for more advanced configurations (inan run farmer-only, wallet-only, timelord-only)
...
- `inan show -w` now correctly outputs all wallet types and balances from the local wallet.
...
- `inan show -b` now returns plot public key and pool public key for each block.
- Added cbor2 binary wheels for ARM64 to the Inan simple site. Raspberry Pi should be just a little easier to install.

### Changed

- Wallet addresses and other key related elements are now expressed in Chech32 which is the Inan implementation of [Bech32](https://github.com/bitcoin/bips/blob/master/bip-0173.mediawiki). All of your old wallet addresses will be replaced with the new Chech32 addresses. The only thing you can't do is send test inan between 1.8/1.9 and 1.10 software. Anyone who upgrades to 1.10 will keep their transactions and balances of test inan from the earlier two releases however.
...
- The proof of space document in inanpos has been updated to the new format and edited for clarity. Additionally GitHub actions now has the on demand ability to create the PDF version.
...
- `inan show -w` now displays a message when balances cannot be displayed instead of throwing an error. Thanks to @freddiecoleman for this fix!
- Fix issue with shutting down full node (full node processes remained open, and caused a spinner when launching Inan)
- Various code review alerts for comparing to a wider type in inanpos were fixed. Additionally, unused code was removed from inanpos
...
- See wallet balances in command line: `inan show -w`
- Retry opening invalid plots every 20 minutes (so you can copy a large plot into a plot directory.)
- We've added `inan keys sign` and `inan keys verify` to allow farmers to certify their ownership of keys.
...
- `inan start wallet-server` changed to `inan start wallet`, for consistency.
...
- Removed legacy scripts such as inan-stop-server, inan-restart-harvester, etc.
...
are read.
- Updated [blspy](https://github.com/Inan-Network/bls-signatures) to use the
...
- The various sub repositories (inanpos, inanvdf, etc.) now build ARM64 binary
...
- The inan executable is now available if installing from the Windows or MacOS
Graphical installer. Try `./inan -h` from
`~\AppData\Local\Inan-Blockchain\app-0.1.8\resources\app.asar.unpacked\daemon\`
in Windows or
`/Applications/Inan.app/Contents/Resources/app.asar.unpacked/daemon` on MacOS.
...
"inan plots create" and "inan plots check".
- Add plot directories to config.yaml from the cli with "inan plots add".
...
- Linux builds of inanvdf and blspy now use a fresh build of gmp 6.2.1.
...
- The Windows installer is now signed by a Inan Network certificate. It may take some time to develop enough reputation to not warn multiple times during install.
...
- Test plots for the simulation and testing harness now go into `~/.inan/test-plots/`
- We have completely refactored all networking code towards making each Inan service use the same default networking infrastructure and move to websockets as the default networking wire protocol.
- We added additional improvements and more RPCs to the start daemon and various services to continue to make inan start/stop reliable cross platform.
...
- We now codesign the Apple .dmg installer with the Inan Network developer ID on both GitHub Actions and Azure Pipelines. We will be notarizing and distributing the Azure Pipelines version as it's built on MacOS Mojave (10.14.6) for stronger cross version support.
...
- Starting inan now waits for the full node to be active before contacting the introducer.
...
- You can now create or import private keys with a 24 word mnemonic, both in the UI and 'inan keys' command line.
...
- We added a `inan keys` command line program, to see, add, and remove private keys.
...
- We added [lgtm](https://lgtm.com/) source analysis on pull request to the inan-blockchain, inanpos, inanvdf, inanbip158, and bls-library repositories to add some automated security analysis to our ci.
...
- `inan netspace` has been refactored to use the `/get_network_space` RPC. The command
...
- inanpos is now easier to compile with MSVC.
...
- `inan-check-plots` uses the plot root and checks the plots in the same manner as harvester.
- `inan-check-plots` now does not override plots.yaml, which means concurrent plots will properly be added to plots.yaml.
...
- We have added an option to `inan-create-plots` to specify the second temporary directory. Creating a plot is a three step process. First a working file ending in `.dat.tmp` is created. This file is usually 5 times larger than the final plot file. In the later stages of plotting a second temp file is created ending in `.dat.2.tmp` which will grow to the size of the final plot file. In the final step, the `.dat.2.tmp` is copied to the final `.dat` plot file. You can now optionally set the directory for the `.dat.2.tmp` file with the `-2` flag. An example use case is plotting on a ramdisk and writing both the second temp file and the final file out to an SSD - `inan-create-plots -n 1 -k 30 -t /mnt/ramdisk -2 /mnt/SSD -d /mnt/SSD`.

### Changed

- `inan init` properly migrates from previous versions including the k>=32 workaround. Additionally, the farming target key is checked to make sure that it is the valid and correct public key format.
- We have implemented a workaround for the `inan start` issues some were having upon crash or reboot. We will be rebuilding start and stop to be robust across platforms.
- This release re-includes `inan-start-harvester`.
...
- The user interface now refers to inan coins with their correct currency code of XCH.
...
- Building the sub reposities (inanpos, inanvdf, blslibrary) now requires CMake 3.14+.

### Fixed

- There was a regression in Inan Proof of Space ([inanpos](https://github.com/Inan-Network/inanpos)) that came from our efforts to speed up plotting on Windows native. Now k>=32 plots work correctly. We made additional bug fixes and corrected limiting small k size generation.
- There was a bug in Timelord handling that could stop all VDF progress.

### Deprecated

- We have made significant changes to the full node database to make it more reliable and quicker to restart. This requires re-syncing the current chain. If you use `inan init` then sync on first start will happen automatically. "\$INAN_ROOT" users will need to delete `$INAN_ROOT/db/*` before starting Beta 1.5. This also fixes the simulation issue in Beta 1.4 where tips could go "back in time."
...
- Now you can run estimates of the total space currently farming the network. Try `inan netspace -d 12` to run an estimate over the last 12 blocks which is approximately 1 hour.
...
- The script `inan start wallet-gui` has been chaned to `inan start wallet` which launches but the GUI and server on MacOS and Linux. `inan start wallet-server` remains for WSL 2 and Windows native.
...
- Currently, there is no way to restore a Coloured Coin Wallet.
- The `inan stop all` command sometimes fails, use `inan-stop-all` instead. In windows, use the task manager to stop the servers.
...
- We added full block lookup to `inan show`.

### Changed

- `inan-restart-harvester` has been renamed from `inan-start-harvester` to better reflect its functionality. Use it to restart a harvester that's farming so that it will pick up newly finished plots.
...
- If you previously used the plot_root variable in config, your plot directory names might not migrate correctly. Please double check the filenames in `~/.inan/beta-1.0b3/config/plots.yaml` after migrating
...
- There is now full transaction support on the Inan blockchain. In this initial Beta 1.0 release, all transaction types are supported though the wallets and UIs currently only directly support basic transactions like coinbase rewards and sending coins while paying fees. UI support for our [smart transactions](https://github.com/Inan-Network/wallets/blob/main/README.md) will be available in the UIs shortly.
...
- The libraries inan-pos, inan-fast-vdf, and inan-bip-158 have been moved to their own repositories: [inanpos](https://github.com/Inan-Network/inanpos), [inanvdf](https://github.com/Inan-Network/inanvdf), and [chaibip158](https://github.com/Inan-Network/inanbip158). They are brought in by inan-blockchain at install time. Our BLS signature library remains at [bls-signatures](https://github.com/Inan-Network/bls-signatures).
- The install process now brings in inanpos, inanvdf, etc from Pypi where they are auto published via GitHub Actions ci using cibuildwheel. Check out `.github/workflows/build.yml` for build methods in each of the sub repositories.
...
- `install-timelord.sh` has been pulled out from `install.sh`. This script downloads the source python package for inanvdf and compiles it locally for timelords. vdf_client can be included or excluded to make building normal user wheels easier.
...
- If you want to benchmark your CPU as a VDF you can use vdf_bench square_asm 500000 for the assembly optimized test or just vdf_bench square 500000 for the plain C++ code path. This tool is found in lib/inanvdf/fast_vdf/.
...
- VDF improvements - VDF verification and generation speed has increased and dependence on flint2 has been removed. We wish to thank Dr. William Hart (@wbhart) for dual licensing parts of his contributions in FLINT and Antic for inclusion in the Inan blockchain.
...
- Added color logging
- Now inan_harvester will periodically announce which plots it is currently farming and their k sizes.

### Changed

- Moved the ssh UI to use RPC.
- Changed the displayed process names for harvester, farmer, fullnode, timelords, and VDFs to to inan_full node, inan_harvester, etc. Fixed a bug that could cause inadvertent shutdown of other processes like an ongoing plotting session when new inan services were started.
